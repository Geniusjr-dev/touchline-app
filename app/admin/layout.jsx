...
export default function AdminLayout({ children }) {
  ...
  useEffect(() => {
    ...
    if (!loading && user && role === "scorer" && (path.startsWith("/admin/teams") || path.startsWith("/admin/competitions"))) router.replace("/admin/matches");
  ...
  if (role === "scorer" && (path.startsWith("/admin/teams") || path.startsWith("/admin/competitions"))) return <Shell><div style={{ color: "#8E939B" }}>Opening your assigned matches.</div></Shell>;
    ...
    <div style={{ minHeight: "100vh", background: "#0A0A0A", color: "#fff" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "14px 18px", borderBottom: "1px solid #26282B", position: "sticky", top: 0, background: "#0A0A0A", zIndex: 10, flexWrap: "wrap" }}>
        ...
        <nav style={{ display: "flex", gap: 14, marginLeft: 8 }}>
          ...
          {role === "admin" && <Tab href="/admin/competitions" path={path}>Competitions</Tab>}
          <Tab href="/admin/matches" path={path}>Matches</Tab>
        </nav>
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 12 }}>
          {memberships.length > 1 && (
            <select value={activeOrganizationId} onChange={(e) => setActiveOrganization(e.target.value)} aria-label="Active organization"
