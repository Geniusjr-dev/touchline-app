...
export default function Teams() {
  ...
  return (
    <div>
      <h1 style={{ fontSize: 22, fontWeight: 800, marginBottom: 16 }}>Teams & players</h1>
      ...
      <div style={{ ...card, padding: 0 }}>
        ...
        {teams.map((tm) => (
          <div key={tm.id} style={{ borderTop: "1px solid #26282B" }}>
            <button onClick={() => setOpenTeam(openTeam === tm.id ? null : tm.id)}
              style={{ width: "100%", display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", background: "transparent", border: "none", cursor: "pointer" }}>
              ...
              <span style={{ fontSize: 15, fontWeight: 600, color: "#fff", flex: 1, textAlign: "left" }}>{tm.name}</span>
              <span style={{ color: "#8E939B", fontSize: 13 }}>{openTeam === tm.id ? "Hide squad ▲" : "Squad ▼"}</span>
            </button>
            {openTeam === tm.id && <Squad teamId={tm.id} />}
          </div>
        ))}
      </div>
    </div>
