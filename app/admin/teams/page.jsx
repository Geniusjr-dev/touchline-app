...
export default function Teams() {
  ...
  return (
    <div>
      ...
      <p style={{ color: "var(--admin-dim)", fontSize: 13, margin: "0 0 16px" }}>Add a team here. Select its card below to manage its profile, squad and trophies on a dedicated page.</p>
      ...
      <div style={{ ...card, padding: 0, overflow: "hidden" }}>
        {teams.length === 0 && <div style={{ color: "var(--admin-dim)", padding: 20, fontSize: 14 }}>No teams yet.</div>}
        {teams.map((team, index) => (
          <Link key={team.id} href={`/admin/teams/${team.id}`} className="flex items-center" style={{ gap: 12, minHeight: 66, padding: "11px 16px", borderTop: index ? "1px solid var(--admin-divider)" : "none", color: "var(--admin-text)" }}>
            ...
            <span style={{ flex: 1, minWidth: 0 }}>
              ...
              <span className="block truncate" style={{ color: "var(--admin-dim)", fontSize: 12, marginTop: 3 }}>{team.display_name || team.country || "Team profile"}</span>
            </span>
            <span className="inline-flex items-center gap-1" style={{ color: "var(--admin-dim)", fontSize: 12 }}>Manage <ChevronRight size={17} /></span>
          </Link>
        ))}
      </div>
    </div>
...
function NewTeamLogoPicker({ file, onChange, fallback, color }) {
  ...
  return <Field label="Team crest"><div style={{ display: "flex", alignItems: "center", gap: 8 }}><AdminImage src={preview} fallback={fallback} color={color} /><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => onChange(event.target.files?.[0] || null)} style={{ color: "var(--admin-dim)", fontSize: 11, maxWidth: 180 }} /></div></Field>;
}
