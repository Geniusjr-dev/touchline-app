      ...
      {team && (
        <div style={{ ...card, padding: 0, overflow: "hidden" }}>
          <div className="flex items-center gap-3" style={{ padding: "16px", borderBottom: "1px solid #26282B" }}>
            <Crest logo={team.logo_url} color={team.color} label={team.display_name || team.name} size={48} ring="#32363C" />
...
function TeamEditor({ team, onSaved }) {
  ...
  async function save(event) {
    ...
    if (!values.logo_url) {
      setMessage("Upload the team crest before saving.");
      return;
    }
    setSaving(true);
    setMessage("");
    const { error } = await updateTeamProfile(team.id, { ...values, name: values.name.trim(), short: team.short || "TEAM" });
    setSaving(false);
  ...
  return (
    <form onSubmit={save} style={editor}>
      ...
      <div style={{ ...formRow, marginTop: 12 }}>
        ...
        <ImageUpload teamLogo label="Team crest" src={values.logo_url} onFile={(file) => upload(file, "logo_url", "crests")} color={values.color} name={values.display_name || values.name} />
      </div>
