  ...
  const tableResult = m.competition_id && competition?.competition_type !== "friendly"
    ...
    : {
        ...
        meta: {
          ...
          format: competition?.competition_type || "friendly",
          groupNumber: null,
          groupLabel: null,
        },
      };
...
async function buildCompetitionTable(competition, requestedGroupNumber = null, homeTeamId = null) {
  ...
  const eligibleMemberships = format === "tournament"
    ? memberships.filter((entry) => Number(entry.group_number) === Number(groupNumber))
    : memberships;
  let teamIds = [...new Set(eligibleMemberships.map((entry) => entry.team_id).filter(Boolean))];

  let matchQuery = supabase
    .from("matches")
    .select("id,home_id,away_id,status,home_score,away_score,group_number,created_at")
    .eq("competition_id", competitionId)
    .order("created_at");
  ...
  if (!teamIds.length) {
    teamIds = [...new Set(matches.flatMap((match) => [match.home_id, match.away_id]).filter(Boolean))];
  }
  const meta = {
  ...
  const rows = {};
  teams.forEach((team) => {
    rows[team.id] = { id: team.id, name: team.display_name?.trim() || team.name, short: team.short, color: team.color, pl: 0, w: 0, d: 0, l: 0, gf: 0, ga: 0, pts: 0, form: [] };
  });
...
export async function listCompetitionTeams(competitionId) {
  ...
  return throwOnError(await supabase
    .from("competition_teams")
    .select("competition_id,team_id,group_number,team:team_id(id,name,display_name,short,color)")
    .eq("competition_id", competitionId)
