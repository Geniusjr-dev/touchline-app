...
function matchLineupQuery(matchId) {
  return supabase
    .from("match_lineups")
    .select("match_id,team_id,player_id,role,formation,sort_order,slot_index,player:player_id(id,name,number,position,country,photo_url)")
    .eq("match_id", matchId)
    .order("role")
    .order("sort_order");
}
...
async function loadMatch(id) {
  ...
  const h2hQuery = supabase
    .from("matches")
    .select("id,home_id,away_id,status,home_score,away_score,match_date,kickoff,competition_id,competition:competition_id(name)")
    .eq("status", "ft")
    .neq("id", id)
    .or(`and(home_id.eq.${m.home_id},away_id.eq.${m.away_id}),and(home_id.eq.${m.away_id},away_id.eq.${m.home_id})`)
  ...
  const h2h = (h2hResult.error ? [] : h2hResult.data || []).map((meeting) => ({
    id: meeting.id,
    ...
    competitionName: meeting.competition?.name || "Match",
  }));
  const teamsById = {};
  teams.forEach((t) => { teamsById[t.id] = publicTeam(t); });
