...
function matchLineupQuery(matchId) {
  return supabase
    .from("match_lineups")
    .select("match_id,team_id,player_id,role,formation,sort_order,player:player_id(id,name,number,position,country,photo_url)")
    .eq("match_id", matchId)
    .order("role")
    .order("sort_order");
}
...
async function loadMatch(id) {
  ...
  const [teamResult, eventResult, compResult, statsResult, lineupResult] = await Promise.all([
  ...
  return { match: shaped, teams: teamsById, detail: { events: evs, table: [], stats, lineups } };
}
...
export async function saveMatchLineup(matchId, teamId, lineup) {
  const result = await supabase.rpc("save_match_lineup", {
    p_match_id: matchId,
    p_team_id: teamId,
