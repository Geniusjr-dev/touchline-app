...
function buildCompetitionPlayerStats(players, matches, events, lineups, teamsById) {
  ...
  lineups.forEach((lineup) => {
    if (completedIds.has(lineup.match_id)) stats.get(lineup.player_id)?.appearances.add(lineup.match_id);
  });
  events.filter((event) => completedIds.has(event.match_id)).forEach((event) => {
    ...
    if (player) {
      ...
      if (event.type === "goal" && event.goal_type !== "own_goal") record.goals += 1;
