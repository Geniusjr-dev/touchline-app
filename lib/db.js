...
function shapeMatch(m, teamsById, events) {
  const hasManual = m.score_home_manual != null && m.score_away_manual != null;
  return {
    id: m.id, home: m.home_id, away: m.away_id, status: m.status,
    min: liveMinute(m),
    hs: hasManual ? m.score_home_manual : goals(events, "home"),
    as: hasManual ? m.score_away_manual : goals(events, "away"),
    time: m.kickoff,
  };
}
...
export async function getMatch(id) {
  ...
  shaped.stoppage_seconds = m.stoppage_seconds;
  shaped.score_home_manual = m.score_home_manual;
  shaped.score_away_manual = m.score_away_manual;
...
export const addCompetition = (name, sub, format, numTeams, numGroups) => supabase.from("competitions").insert({ name, sub, format: format || "tournament", num_teams: numTeams || null, num_groups: numGroups || null });
...
export const updateMatch = (id, patch) => supabase.from("matches").update(patch).eq("id", id);
// pause / resume clock (freeze)
export const pauseClock = (id, elapsed) => supabase.from("matches").update({ clock_running: false, elapsed_seconds: elapsed, clock_started_at: null }).eq("id", id);
export const resumeClock = (id) => supabase.from("matches").update({ clock_running: true, clock_started_at: new Date().toISOString() }).eq("id", id);
// stoppage (injury) minutes
export const setStoppage = (id, seconds) => supabase.from("matches").update({ stoppage_seconds: Math.max(0, seconds | 0) }).eq("id", id);
// manual score override (emergency desync correction)
export const setManualScore = (id, h, a) => supabase.from("matches").update({ score_home_manual: h, score_away_manual: a }).eq("id", id);
export const clearManualScore = (id) => supabase.from("matches").update({ score_home_manual: null, score_away_manual: null }).eq("id", id);
...
export async function getStandings(competitionId) {
  ...
  (matches || []).filter((m) => m.status === "ft").forEach((m) => {
    ...
    const hg = manual ? m.score_home_manual : goalsFor(m.id, "home"), ag = manual ? m.score_away_manual : goalsFor(m.id, "away");
