...
function publicTeam(team) {
  ...
  return { name: displayName || team.name, fullName: team.name, displayName, short: team.short, color: team.color };
}
...
function shapeMatch(m, events, competition = null) {
  return {
    id: m.id, home: m.home_id, away: m.away_id, competitionId: m.competition_id, organizationId: m.organization_id,
    status: m.status,
    hs: m.home_score ?? goals(events, "home"),
    as: m.away_score ?? goals(events, "away"),
    date: m.match_date || null, time: m.kickoff,
    ...
    locked_at: m.locked_at, reopened_at: m.reopened_at,
  };
}
...
export async function getMatch(id) {
  ...
  const evs = events.map((e) => {
    ...
    const eventPeriodEnd = periodEndMinute({ current_period: e.period || 1, matchDurationMinutes: shaped.matchDurationMinutes, extraTimeMinutes: shaped.extraTimeMinutes });
    const minuteLabel = eventPeriodEnd != null && displayMinute > eventPeriodEnd ? `${eventPeriodEnd}+${displayMinute - eventPeriodEnd}'` : `${displayMinute}'`;
    return {
      id: e.id, m: Number(e.period || 1) * 100000 + Number(e.elapsed_seconds ?? displayMinute * 60),
      min: minuteLabel, displayMinute, elapsedSeconds: e.elapsed_seconds ?? displayMinute * 60, period: e.period || 1,
      type: e.type, side: e.side, player: e.player, playerId: e.player_id, assist: e.assist,
  ...
  const tableResult = m.competition_id && competition?.competition_type !== "friendly"
    ? await buildCompetitionTable(competition, m.group_number, m.home_id)
    : { rows: [], meta: { hasTable: false, competitionName: competition?.name || null, format: competition?.competition_type || "friendly", groupNumber: null, groupLabel: null } };
  const statsRow = throwOnError(await supabase.from("match_stats").select("*").eq("match_id", id).maybeSingle());
  const stats = statsRow && (Object.keys(statsRow.home || {}).length || Object.keys(statsRow.away || {}).length)
    ? { home: statsRow.home || {}, away: statsRow.away || {} } : null;
  return { match: shaped, teams: teamsById, detail: { events: evs, table: tableResult.rows, tableMeta: tableResult.meta, stats } };
}

function runningScoreAt(events, upto) {
  let home = 0, away = 0;
  for (const event of events) {
    if (event.type === "goal") { if (event.side === "home") home += 1; else away += 1; }
    if (event.id === upto.id) break;
  }
  return { home, away };
}

async function buildCompetitionTable(competition, requestedGroupNumber = null, homeTeamId = null) {
  ...
  const memberships = throwOnError(await supabase.from("competition_teams").select("team_id,group_number").eq("competition_id", competitionId)) || [];
  if (format === "tournament" && !groupNumber && homeTeamId) {
    groupNumber = Number(memberships.find((entry) => entry.team_id === homeTeamId)?.group_number || 0) || null;
  }
  const eligibleMemberships = format === "tournament" ? memberships.filter((entry) => Number(entry.group_number) === Number(groupNumber)) : memberships;
  let teamIds = [...new Set(eligibleMemberships.map((entry) => entry.team_id).filter(Boolean))];
  let matchQuery = supabase.from("matches").select("id,home_id,away_id,status,home_score,away_score,group_number,created_at").eq("competition_id", competitionId).order("created_at");
  ...
  const meta = {
    hasTable: format === "league" || (format === "tournament" && Boolean(groupNumber)),
    competitionName: competition.name, format, groupNumber,
    groupLabel: groupNumber ? `Group ${String.fromCharCode(64 + groupNumber)}` : null,
  };
  if (!teamIds.length) return { rows: [], meta };
  const teams = throwOnError(await supabase.from("teams").select("id,name,display_name,short,color").in("id", teamIds)) || [];
  const rows = {};
  teams.forEach((team) => { rows[team.id] = { id: team.id, name: team.display_name?.trim() || team.name, short: team.short, color: team.color, pl: 0, w: 0, d: 0, l: 0, gf: 0, ga: 0, pts: 0, form: [] }; });
  matches.filter((match) => match.status === "ft").forEach((match) => {
    const home = rows[match.home_id]; const away = rows[match.away_id];
    if (!home || !away) return;
    const homeScore = match.home_score || 0; const awayScore = match.away_score || 0;
    home.pl += 1; away.pl += 1; home.gf += homeScore; home.ga += awayScore; away.gf += awayScore; away.ga += homeScore;
    if (homeScore > awayScore) { home.w += 1; home.pts += 3; home.form.push("W"); away.l += 1; away.form.push("L"); }
    else if (homeScore < awayScore) { away.w += 1; away.pts += 3; away.form.push("W"); home.l += 1; home.form.push("L"); }
    else { home.d += 1; home.pts += 1; home.form.push("D"); away.d += 1; away.pts += 1; away.form.push("D"); }
  });
  return { rows: Object.values(rows).sort((a, b) => b.pts - a.pts || (b.gf - b.ga) - (a.gf - a.ga) || b.gf - a.gf || a.name.localeCompare(b.name)), meta };
}
...
export async function getMyAccess(userId) {
  ...
  return { profile: throwOnError(profileResult), memberships: throwOnError(membershipResult) || [] };
}
...
export async function listCompetitionTeams(competitionId) {
  ...
  return throwOnError(await supabase.from("competition_teams").select("competition_id,team_id,group_number,team:team_id(id,name,display_name,short,color)").eq("competition_id", competitionId).order("group_number", { ascending: true, nullsFirst: true })) || [];
}
export async function listMatches(organizationId, role, userId) {
  ...
  let query = supabase.from("matches").select("*, home:home_id(name,display_name,short,color), away:away_id(name,display_name,short,color)").order("created_at", { ascending: false });
...
export const addCompetition = (organization_id, name, sub, match_duration_minutes = 90, competition_type = "tournament", group_count = 0, teams_per_group = 0) =>
  supabase.from("competitions").insert({ organization_id, name, sub, match_duration_minutes, competition_type, group_count, teams_per_group });
export const updateCompetition = (id, name, sub, match_duration_minutes, competition_type, group_count, teams_per_group) =>
  supabase.from("competitions").update({ name, sub: sub?.trim() || null, match_duration_minutes, competition_type, group_count, teams_per_group }).eq("id", id);
...
export const createMatch = (organization_id, competition_id, home_id, away_id, kickoff, match_date, group_number = null) =>
  supabase.from("matches").insert({ organization_id, competition_id, home_id, away_id, kickoff, match_date, group_number, status: "scheduled" });
export const deleteMatch = (id) => supabase.from("matches").delete().eq("id", id);
...
export async function listMatchScorers(matchIds) {
  ...
  return rows.reduce((map, row) => { (map[row.match_id] ||= []).push(row.user_id); return map; }, {});
}
...
// ---------- checked match mutations (RPC) ----------
export const transitionMatchStatus = (matchId, status) => supabase.rpc("transition_match_status", { p_match_id: matchId, p_status: status });
export const setStatus = transitionMatchStatus;
export const reopenMatch = (matchId, reason) => supabase.rpc("reopen_match", { p_match_id: matchId, p_reason: reason });
export const setMatchStoppageTime = (matchId, minutes) => supabase.rpc("set_match_stoppage_time", { p_match_id: matchId, p_minutes: minutes });
export const recordMatchEvent = (matchId, event) => supabase.rpc("record_match_event", {
  p_match_id: matchId, p_type: event.type, p_side: event.side,
  p_player_id: event.player_id || null, p_player: event.player || null, p_assist: event.assist || null,
});
export const deleteMatchEvent = (eventId) => supabase.rpc("delete_match_event", { p_event_id: eventId });
...
export const STAT_DEFS = [
  { key: "possession", label: "Ball possession", pct: true },
...
export async function getMatchStats(id) {
  const row = throwOnError(await supabase.from("match_stats").select("*").eq("match_id", id).maybeSingle());
  return row || null;
}
export const upsertMatchStats = (id, home, away) =>
  supabase.from("match_stats").upsert({ match_id: id, home, away, updated_at: new Date().toISOString() }, { onConflict: "match_id" });
