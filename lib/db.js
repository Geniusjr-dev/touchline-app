...
function isUnpublishedRetrospectiveMatch(match) {
  return match?.operation_mode === "retrospective" && match.status !== "ft";
}

function publicMatchRecord(match) {
  if (!isUnpublishedRetrospectiveMatch(match)) return match;
  return {
    ...match,
    status: "scheduled",
    home_score: null,
...
function publicMatchEvents(match, events = []) {
  return isUnpublishedRetrospectiveMatch(match) ? [] : events;
}
...
async function loadLeaguesHome() {
  ...
  const [competitionResult, matchResult, membershipResult] = await Promise.all([
    ...
    supabase.from("matches").select("id,competition_id,home_id,away_id,status,operation_mode,match_date,kickoff").order("match_date").order("kickoff"),
  ...
  return {
    competitions: competitions.map((competition) => {
      ...
      return {
        ...
        liveCount: competitionMatches.filter((match) => ["live", "ht", "et_live", "et_ht"].includes(publicMatchRecord(match).status)).length,
      };
    }),
  };
}
...
function shapeMatch(m, events, competition = null) {
  const match = publicMatchRecord(m);
  const visibleEvents = publicMatchEvents(m, events);
  return {
    id: match.id,
    home: match.home_id,
    away: match.away_id,
    ...
    status: match.status,
    hs: match.home_score ?? goals(visibleEvents, "home"),
    as: match.away_score ?? goals(visibleEvents, "away"),
    date: match.match_date || null,
    time: match.kickoff,
    clock_elapsed_seconds: match.clock_elapsed_seconds ?? (match.clock_base || 0) * 60,
    clock_started_at: match.clock_started_at,
    current_period: match.current_period || 0,
    operationMode: match.operation_mode || "live",
    ...
    venueLocation: match.venue_location || null,
    venueCapacity: match.venue_capacity || null,
    venueSurface: match.venue_surface || null,
    weather: match.weather || null,
    refereeName: match.referee_name || null,
    homeKitColor: match.home_kit_color || null,
    awayKitColor: match.away_kit_color || null,
    first_half_stoppage_minutes: match.first_half_stoppage_minutes || 0,
    second_half_stoppage_minutes: match.second_half_stoppage_minutes || 0,
    extra_time_first_half_stoppage_minutes: match.extra_time_first_half_stoppage_minutes || 0,
    extra_time_second_half_stoppage_minutes: match.extra_time_second_half_stoppage_minutes || 0,
    locked_at: match.locked_at,
    reopened_at: match.reopened_at,
  };
}
...
async function loadMatch(id) {
  ...
  const stats = isUnpublishedRetrospectiveMatch(m)
    ? normalizeMatchStats(null)
    : normalizeMatchStats(throwOnError(statsResult));
  ...
  const teamsById = {};
  teams.forEach((t) => { teamsById[t.id] = publicTeam(t); });
  const shaped = shapeMatch(m, visibleEvents, competition);
  ...
  const evs = visibleEvents.map((e) => {
    const fallbackScore = runningScoreAt(visibleEvents, e);
...
export async function getLeagueCentre(competitionId) {
  ...
  const matches = throwOnError(matchResult) || [];
  const publicMatches = matches.map((match) => publicMatchRecord(match));
  const unpublishedMatchIds = new Set(matches.filter(isUnpublishedRetrospectiveMatch).map((match) => match.id));
  const memberships = membershipResult.error ? [] : membershipResult.data || [];
  ...
  const visibleEvents = events.filter((event) => !unpublishedMatchIds.has(event.match_id));
  const visibleStatistics = statistics.filter((statistic) => !unpublishedMatchIds.has(statistic.match_id));
  const playerStats = buildCompetitionPlayerStats(players, publicMatches, visibleEvents, lineups, teamsById);
  const teamStats = buildCompetitionTeamStats(teamsById, publicMatches, visibleEvents, visibleStatistics);
  const teamOfWeek = buildTeamOfWeek(publicMatches, visibleEvents, lineups, players, teamsById);
  return {
    competition: {
      id: competition.id,
      name: competition.name,
...
async function buildCompetitionTable(competitionId, requestedGroupNumber = null, teamId = null) {
  ...
  let matchQuery = supabase.from("matches")
    .select("id,home_id,away_id,status,operation_mode,home_score,away_score,group_number,match_date,kickoff,created_at")
    .eq("competition_id", competitionId)
  ...
  matches.map((match) => publicMatchRecord(match)).filter((match) => ["live", "ht", "et_live", "et_ht", "ft"].includes(match.status)).forEach((match) => {
...
export async function getTeamCentre(teamId) {
  ...
  return {
    ...
    stats: aggregateTeamStats(publicMatches, statistics.filter((statistic) => !unpublishedMatchIds.has(statistic.match_id)), teamId),
  };
}
