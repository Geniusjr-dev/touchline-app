...
export async function getMatch(id) {
  ...
  const [teamOutcome, eventOutcome, compOutcome, statsOutcome] = await Promise.allSettled([
  ...
  const optionalData = (outcome, fallback) => {
    if (outcome.status !== "fulfilled" || outcome.value?.error) return fallback;
    return outcome.value?.data ?? fallback;
  };
  const teams = optionalData(teamOutcome, []);
  const events = optionalData(eventOutcome, []);
  const competition = optionalData(compOutcome, null);
  const stats = normalizeMatchStats(optionalData(statsOutcome, null));
  const teamsById = {};
  teams.forEach((t) => { teamsById[t.id] = publicTeam(t); });
  ...
  const evs = events.map((e) => {
    ...
    return {
      ...
      assist: e.assist,
      goalType: e.goal_type || (e.type === "goal" ? "direct_goal" : null),
...
export const recordMatchEvent = (matchId, event) =>
  supabase.rpc("record_match_event", {
    ...
    p_goal_type: event.type === "goal" ? (event.goal_type || "direct_goal") : null,
  });
