...
function eventMinute(event) {
  return `${Math.max(1, Number(event?.display_minute ?? event?.minute) || 1)}'`;
}
...
export async function buildMatchNotification(admin, matchId, kind, eventId, previousStatus) {
  const { data: match, error: matchError } = await admin
    ...
    .maybeSingle();
  if (matchError || !match) throw new Error("Match not found.");
  const { data: teams } = await admin.from("teams").select("id,name,display_name,logo_url").in("id", [match.home_id, match.away_id]);
  ...
  if (kind === "event" || kind === "scorer") {
    const { data: event, error: eventError } = await admin
      .from("events")
      .select("id,type,side,player,goal_type,card_type,minute,display_minute")
      .eq("id", eventId)
      .eq("match_id", matchId)
      .maybeSingle();
    if (eventError || !event) throw new Error("Match event not found.");
    const eventTeam = event.side === "home" ? homeName : awayName;
    const eventTeamLogo = event.side === "home" ? home?.logo_url : away?.logo_url;
    if (event.type === "goal") {
      const minute = eventMinute(event);
      if (kind === "scorer") {
        if (!event.player) return null;
        return {
          title: `${eventTeam} Goal`,
          body: `${minute} · ${event.goal_type === "own_goal" ? "Own goal by " : ""}${event.player}`,
          url,
          icon: eventTeamLogo || undefined,
          tag: `match-${matchId}-event-${event.id}-scorer`,
        };
      }
      const scoreline = event.side === "home"
        ? `${homeName} [${Number(match.home_score) || 0}] - ${Number(match.away_score) || 0} ${awayName}`
        : `${homeName} ${Number(match.home_score) || 0} - [${Number(match.away_score) || 0}] ${awayName}`;
      return {
        title: `${eventTeam} Scores`,
        body: `${minute} · ${scoreline}`,
        url,
        icon: eventTeamLogo || undefined,
        tag: `match-${matchId}-event-${event.id}-goal`,
      };
    }
    if (event.type === "red") {
