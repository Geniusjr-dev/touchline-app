...
function hkdfExpand(pseudoRandomKey, info, length) {
  return hmac(pseudoRandomKey, Buffer.concat([Buffer.from(info), Buffer.from([1])])).subarray(0, length);
}
...
function createVapidAuthorization(endpoint, environment) {
  const publicKey = fromBase64Url(environment.publicKey);
  const privateKey = fromBase64Url(environment.privateKey);
  if (publicKey.length !== 65 || publicKey[0] !== 4 || privateKey.length !== 32) {
    throw new Error("The VAPID keys are invalid.");
  }
  const header = base64Url(JSON.stringify({ typ: "JWT", alg: "ES256" }));
  const payload = base64Url(JSON.stringify({
    aud: new URL(endpoint).origin,
    exp: Math.floor(Date.now() / 1000) + 12 * 60 * 60,
    sub: environment.subject,
  }));
  ...
  const signature = sign("sha256", Buffer.from(unsignedToken), { key, dsaEncoding: "ieee-p1363" });
  return `vapid t=${unsignedToken}.${base64Url(signature)}, k=${environment.publicKey}`;
}
...
function encryptPushPayload(subscription, payload) {
  ...
  if (clientPublicKey.length !== 65 || clientPublicKey[0] !== 4 || authSecret.length < 16) {
    throw new Error("The saved push subscription is invalid.");
  }
  ...
  const authenticationKey = hmac(authSecret, sharedSecret);
  const keyInfo = Buffer.concat([
    Buffer.from("WebPush: info\0"),
    clientPublicKey,
    serverPublicKey,
  ]);
  const inputKeyMaterial = hkdfExpand(authenticationKey, keyInfo, 32);
  const salt = randomBytes(16);
  const pseudoRandomKey = hmac(salt, inputKeyMaterial);
  const contentEncryptionKey = hkdfExpand(pseudoRandomKey, Buffer.from("Content-Encoding: aes128gcm\0"), 16);
  const nonce = hkdfExpand(pseudoRandomKey, Buffer.from("Content-Encoding: nonce\0"), 12);

  const cipher = createCipheriv("aes-128-gcm", contentEncryptionKey, nonce);
  const plaintext = Buffer.concat([Buffer.from(encoder.encode(payload)), Buffer.from([2])]);
...
export async function sendWebPush(subscription, message) {
  ...
  const response = await fetch(subscription.endpoint, {
    method: "POST",
    headers: {
      Authorization: createVapidAuthorization(subscription.endpoint, environment),
  ...
  if (!response.ok) {
    const error = new Error(`The push service returned ${response.status}.`);
...
export async function sendMatchNotification(admin, matchId, message) {
  ...
  if (!ids.length || !message) return { sent: 0 };
