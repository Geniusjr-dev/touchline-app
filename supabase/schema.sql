-- Touchline current schema for a brand-new Supabase project.
-- Existing Touchline projects must run 001_match_integrity.sql instead.
...
-- Player squads (included here so fresh installations need only this file).
...
-- Current ownership, security, clock and atomic scoring layer.
...
alter table public.competitions
  add constraint competitions_type_check check (competition_type in ('friendly', 'league', 'tournament'));

do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'competitions_group_settings_check') then
    alter table public.competitions add constraint competitions_group_settings_check check (
      (competition_type = 'tournament' and group_count between 1 and 26 and teams_per_group between 2 and 32)
      or (competition_type in ('friendly', 'league') and group_count = 0 and teams_per_group = 0)
    );
  end if;
end $$;
...
create table if not exists public.competition_teams (
  ...
  constraint competition_teams_group_number_check check (group_number is null or group_number between 1 and 26)
);
...
create index if not exists competition_teams_group_idx on public.competition_teams(competition_id, group_number, team_id);
create index if not exists matches_competition_group_idx on public.matches(competition_id, group_number, created_at);
...
create policy "public read competition teams" on public.competition_teams for select using (true);
create policy "organization admins create competition teams" on public.competition_teams for insert
  with check (exists (select 1 from public.competitions c join public.teams t on t.id = team_id and t.organization_id = c.organization_id where c.id = competition_id and public.is_org_admin(c.organization_id)));
create policy "organization admins update competition teams" on public.competition_teams for update
  using (exists (select 1 from public.competitions c where c.id = competition_id and public.is_org_admin(c.organization_id)))
  with check (exists (select 1 from public.competitions c join public.teams t on t.id = team_id and t.organization_id = c.organization_id where c.id = competition_id and public.is_org_admin(c.organization_id)));
create policy "organization admins delete competition teams" on public.competition_teams for delete
  using (exists (select 1 from public.competitions c where c.id = competition_id and public.is_org_admin(c.organization_id)));
...
begin
  ...
  ) then alter publication supabase_realtime add table public.competition_teams; end if;
end $$;
